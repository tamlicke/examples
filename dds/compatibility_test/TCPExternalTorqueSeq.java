...
/**
* A sequence of TCPExternalTorque instances.
*/
public final class TCPExternalTorqueSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TCPExternalTorqueSeq() {
        super(TCPExternalTorque.class);
    }

    public TCPExternalTorqueSeq (int initialMaximum) {
        super(TCPExternalTorque.class, initialMaximum);
    }

    public TCPExternalTorqueSeq (Collection<?> elements) {
        super(TCPExternalTorque.class, elements);
    }

    public  TCPExternalTorque get(int index) { 
        return (TCPExternalTorque) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TCPExternalTorque.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TCPExternalTorque objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TCPExternalTorque.create());
