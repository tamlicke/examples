...
/**
* A sequence of BrakeTestRemainingTime instances.
*/
public final class BrakeTestRemainingTimeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BrakeTestRemainingTimeSeq() {
        super(BrakeTestRemainingTime.class);
    }

    public BrakeTestRemainingTimeSeq (int initialMaximum) {
        super(BrakeTestRemainingTime.class, initialMaximum);
    }

    public BrakeTestRemainingTimeSeq (Collection<?> elements) {
        super(BrakeTestRemainingTime.class, elements);
    }

    public  BrakeTestRemainingTime get(int index) { 
        return (BrakeTestRemainingTime) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BrakeTestRemainingTime.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BrakeTestRemainingTime objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BrakeTestRemainingTime.create());
