...
/**
* A sequence of CartesianPosition instances.
*/
public final class CartesianPositionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CartesianPositionSeq() {
        super(CartesianPosition.class);
    }

    public CartesianPositionSeq (int initialMaximum) {
        super(CartesianPosition.class, initialMaximum);
    }

    public CartesianPositionSeq (Collection<?> elements) {
        super(CartesianPosition.class, elements);
    }

    public  CartesianPosition get(int index) { 
        return (CartesianPosition) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CartesianPosition.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CartesianPosition objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CartesianPosition.create());
