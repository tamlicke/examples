...
/**
* A sequence of FootControllerPositionEnum instances.
*/
public final class FootControllerPositionEnumSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FootControllerPositionEnumSeq() {
        super(FootControllerPositionEnum.class);
    }

    public FootControllerPositionEnumSeq (int initialMaximum) {
        super(FootControllerPositionEnum.class, initialMaximum);
    }

    public FootControllerPositionEnumSeq (Collection<?> elements) {
        super(FootControllerPositionEnum.class, elements);
    }

    public  FootControllerPositionEnum get(int index) { 
        return (FootControllerPositionEnum) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FootControllerPositionEnum.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FootControllerPositionEnum objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FootControllerPositionEnum.create());
