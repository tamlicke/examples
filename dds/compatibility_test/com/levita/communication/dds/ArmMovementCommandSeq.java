...
/**
* A sequence of ArmMovementCommand instances.
*/
public final class ArmMovementCommandSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ArmMovementCommandSeq() {
        super(ArmMovementCommand.class);
    }

    public ArmMovementCommandSeq (int initialMaximum) {
        super(ArmMovementCommand.class, initialMaximum);
    }

    public ArmMovementCommandSeq (Collection<?> elements) {
        super(ArmMovementCommand.class, elements);
    }

    public  ArmMovementCommand get(int index) { 
        return (ArmMovementCommand) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ArmMovementCommand.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ArmMovementCommand objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ArmMovementCommand.create());
