...
/**
* A sequence of TCPForce instances.
*/
public final class TCPForceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TCPForceSeq() {
        super(TCPForce.class);
    }

    public TCPForceSeq (int initialMaximum) {
        super(TCPForce.class, initialMaximum);
    }

    public TCPForceSeq (Collection<?> elements) {
        super(TCPForce.class, elements);
    }

    public  TCPForce get(int index) { 
        return (TCPForce) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TCPForce.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TCPForce objects (beyond the original size of this object)
